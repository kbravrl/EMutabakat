...
namespace EMutabakat.Services
{
    public class DovizKoduService : IDovizKoduService
    {
        ...
        public DovizKoduService(IDbContextFactory<AppDbContext> contextFactory)
        {
        ...
        public async Task<DovizKodu?> UpdateAsync(DovizKodu dovizKodu, string originalTcmb)
        {
            ...
            var existing = await context.DovizKodlari.FirstOrDefaultAsync(x => x.TCMB == normalizedOriginal);
            if (existing == null)
                return null;
            ...
            if (!string.Equals(normalizedOriginal, dovizKodu.TCMB, StringComparison.Ordinal))
            {
                ...
                await context.Cariler
                    ...
                    .ExecuteUpdateAsync(setters => setters.SetProperty(c => c.CariDovizKodu, dovizKodu.TCMB));

                await context.Mutabakatlar
                    .Where(m => m.MutabakatDovizKodu == normalizedOriginal)
                    .ExecuteUpdateAsync(setters => setters.SetProperty(m => m.MutabakatDovizKodu, dovizKodu.TCMB));
        ...
        public async Task<bool> DeleteAsync(string tcmb)
        {
            ...
            var entity = await context.DovizKodlari.FirstOrDefaultAsync(x => x.TCMB == normalized);
            if (entity == null)
                return false;
